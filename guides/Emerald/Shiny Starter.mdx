...
<ShowIf slug="/rs-shiny-starter">
  ## Requirements

  Your battery must be dead. 
</ShowIf>
...
<ShowIf slug="/rs-shiny-starter">
  The strategy to determine the SID is to attempt a timing of 1633 advances. By knowing the TID and that the actual hit advance is near 1633,
we can generate a list of possible SIDs (one SID for each advance near 1633).
</ShowIf>

<ShowIf slug="/emerald-shiny-starter">
  The strategy to determine the SID is to attempt a timing of 1410 advances. By knowing the TID and that the actual hit advance is near 1410,
we can generate a list of possible SIDs (one SID for each advance near 1410).
</ShowIf>


Afterwards, each possible SID of the list must be validated until the right one is found. A SID can be validated by catching a Pokémon that would be shiny is the player has that SID. If that Pokémon is shiny, then the tested SID is the right one. Otherwise it isn't.
...
If the Pokémon is not shiny, then its associated SID is not your SID.
...
5. Quickly interact with the bag to avoid unwanted advance from wandering NPCs. 
6. Select your starter and wait wit the confirmation message `Do you choose this POKEMON?` displayed.
7. Exactly when the second timer reaches 0, press `A` to select `YES`.
...
10. If it's not shiny, fill the species, gender, nature, and stats in the form below, then press "Find advances matching caught starter Pokémon".
11. <ShowIf slug="/rs-shiny-starter">
      If no results is shown, a field is incorrect, or the battery is not dead.
    </ShowIf>
    <ShowIf slug="/emerald-shiny-starter">
