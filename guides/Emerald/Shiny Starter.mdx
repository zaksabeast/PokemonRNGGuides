...
<ShowIf slug="/rs-shiny-starter">
  The strategy to determine the SID is to attempt a timing of 1633 advances. By
  knowing the TID and that the actual hit advance is near 1633, we can generate
  a list of possible SIDs (one SID for each advance near 1633).
</ShowIf>

<ShowIf slug="/emerald-shiny-starter">
  The strategy to determine the SID is to attempt a timing of 1410 advances. By
  knowing the TID and that the actual hit advance is near 1410, we can generate
  a list of possible SIDs (one SID for each advance near 1410).
</ShowIf>
...
5. Quickly open the bag to avoid unwanted advance from wandering NPCs.
6. Select your starter and wait with the confirmation message `Do you choose this POKEMON?` displayed.
7. Exactly when the second timer reaches 0, press `A` to select `YES` to choose your starter.
