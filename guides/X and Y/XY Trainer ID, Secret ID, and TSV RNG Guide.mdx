---
title: "Trainer ID, Secret ID, and/or TSV RNG"
description: "RNG for a specific Trainer ID, SID, or TSV"
slug: "pcalc-xy-tid"
subCategory: "Custom Firmware"
---

## Tools

You need a new save file to RNG the Trainer ID, Secret ID, or Trainer Shiny Value. To delete an existing save file, press `X + B + Up` on the d-pad at the title screen. This deletes the current save file and brings you to the language select screen.

Make sure to back up your save file using a save file manager like [Checkpoint](https://github.com/FlagBrew/Checkpoint/releases) if you want to return to a previous save file.

## Tools

- A 3DS with PCalc ([PCalc Install Guide](https://www.pokemonrng.com/misc-3ds-installing-pcalc))
- [3DSRNGTool](https://github.com/wwwwwwzx/3DSRNGTool/releases)
- (Optional) A network connection.

## Step 1: 3DSRNGTool Setup

1. In the upper right corner, select your game version and click the "Advanced" option. Seed and TSV can be blank.
2. In the "ID RNG" tab, fill in your desired TID, SID, or TSV. 3DSRNGTool can search for parts of numbers or the full number. It is recommended to RNG for only one at a time.
3. Under "Frame Range," start from "0".

![Setup](/images/X-Y/TID/Setup.png)

## Step 2: Game Setup

```
Note: If you use a N3DS or N2DSXL, you can speed up the game when the PCalc overlay is active by enabling L2 cache + 804 MHz in the Rosalina menu options (press `Select + L + Down` to access).
```

1. Load the game and connect to NTR Helper in 3DSRNGTool. Instructions are found [here]. If you can't connect to the internet, fill in the tiny seeds manually later.
2. Select the game language.
3. Continue playing until you reach the final screen shown below.
4. Pause the game by pressing `Start + Select` on that screen.

![Final Screen](/images/X-Y/TID/Final-Screen.png)

This is the final screen to change character selection.

## Step 3: Finding a Target Frame

The frames in 3DSRNGTool track 4 values known as tiny seeds. These seeds generate trainer info like TID, SID, and TSV. Each advancement in tiny seeds equals a frame advancement. Note that these frames differ from those in PCalc.

In X/Y, tiny seeds advance throughout the introduction at a random pace, but in a predictable order that 3DSRNGTool can track.

![NTR Helper](/images/X-Y/TID/NTR-Helper.png)

1. Press "Calculate" in the main window to get a list of frames available for RNG.
2. Adjust the range as needed. Higher frames take longer to obtain the desired TID/SID/TSV.

![Target](/images/X-Y/TID/Target.png)

_In this example, we will get the highlighted trainer data._

3. If the target frame is too high or the TID/SID/TSV combination is not possible with your initial seed, reboot the system for a different initial seed. Press `Select + L + Down` in the Rosalina menu to access the reboot option.

```
Note: Soft resetting the game will hard lock the system due to NTR! You must do a hard reset of the console.
```

## Step 4: Advancing the Tiny Seeds

1. Unpause the game by pressing `Start`.
2. After confirming your character selection, advance to this screen.
   ![Advance Screen](/images/X-Y/TID/Advance.png)
   - This is the last screen for user input before generating trainer data.
3. Allow tiny seeds to advance on this final screen.
   - Pause the game with `Start + Select` to compare tiny seeds in 3DSRNGTool every few minutes to avoid missing your target.
   - You can update tiny seeds in 3DSRNGTool and press "Calculate" to track progress.
   - If NTR Helper is running, it will automatically update tiny seeds in 3DSRNGTool.
   - Disable filters once the target frame is about 30 frames away for a more accurate count.
4. When your desired TID/SID/TSV is about 12 frames away from the current seeds, pause the game using `Start + Select`.
5. Advance frames and tiny seeds by pressing `Select` while the game is paused.
   - The frame counter in PCalc should go up by one for every two presses of `Select`.
6. Once on the correct frame, hold `A` so you unpause at that exact frame to hit the right tiny seeds.
   - Make sure to do this on the first main RNG frame of your target (TinyMT) frame.
7. After gaining control of your character, check your TID via the trainer card.

![Success](/images/X-Y/TID/Success.png)

```
Note: PCalc can display TSV information by pressing `Start + Right` and then `Select + Left`.
```
