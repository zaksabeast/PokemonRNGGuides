...
use crate::gen3::{calculate_cycle_data, CycleRange, Gen3Method, Wild3EncounterTable, Wild3SearcherResultMon };
...
pub fn generate_gen3_wild_distribution(
    ...
    assert_eq!(opts.methods, vec![Gen3Method::Wild1,Gen3Method::Wild2,Gen3Method::Wild3,Gen3Method::Wild4,Gen3Method::Wild5]);
    ...
    let search_results = gen_results
        ...
        .sorted_by(|lhs,rhs|{
            let lhs_start = lhs.cycle_data_by_lead.as_ref().unwrap().post_sweet_scent_range.start.cycle;
            let rhs_start = rhs.cycle_data_by_lead.as_ref().unwrap().post_sweet_scent_range.start.cycle;
            rhs_start.cmp(&lhs_start)
        })
        .collect::<Vec<_>>();

        search_results.iter().enumerate().map(|(i,searcher_res)|{
            let cycle_data = calculate_cycle_data(
                &searcher_res.cycle_data_by_lead.as_ref().unwrap().post_sweet_scent_range,
                lead_cycle_speed,
            );
            if searcher_res.method != Gen3Method::Wild5 {
                Wild3MethodDistributionResult {
                    searcher_res:searcher_res.clone(),
                    pre_sweet_scent_cycle_ranges:vec![cycle_data.pre_sweet_scent_cycle_range],
                ...
                let mut pre_sweet_scent_cycle_ranges: Vec<CycleRange<usize>> = vec![
                    CycleRange::<usize> {
                ...
                for other_res_idx in i + 1..search_results.len() {
                    ...
                    let other_cycle_data = calculate_cycle_data(
                        &other_res.cycle_data_by_lead.as_ref().unwrap().post_sweet_scent_range,
                        lead_cycle_speed,
                    );

                    let end_of_previous = other_cycle_data.pre_sweet_scent_cycle_range.start;
                    pre_sweet_scent_cycle_ranges.last_mut().unwrap().len = end_of_previous.saturating_sub(pre_sweet_scent_cycle_ranges.last_mut().unwrap().start);
                ...
                let end = cycle_data.pre_sweet_scent_cycle_range.end();
                let start = pre_sweet_scent_cycle_ranges.last_mut().unwrap().start;
                pre_sweet_scent_cycle_ranges.last_mut().unwrap().len = end.saturating_sub(start);

                pre_sweet_scent_cycle_ranges.retain(|res|{
                    res.len > 0
                });
...
mod test {
    ...
    use crate::gen3::{
        Gen3Lead, Gen3Method, Wild3EncounterTable,
    ...
    fn test_distribution_generator() {
        let opts = Wild3GeneratorOptions {
            ...
            methods: vec![Gen3Method::Wild1, Gen3Method::Wild2, Gen3Method::Wild3, Gen3Method::Wild4, Gen3Method::Wild5],
            ..Default::default()
        };
        let result = generate_gen3_wild_distribution(
            0,
            &opts,
            &Wild3EncounterTable::default(),
            775);
