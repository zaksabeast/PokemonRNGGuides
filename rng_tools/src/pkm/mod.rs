...
pub use stat_calculator::{calculate_non_hp, calculate_hp};
    ...
    pub fn pass_filter_stats(&self, state: &impl PkmState) -> bool {
        match (&self.stats) {
            ...
            Some(stats_filter) => {
                ...
                let actual_atk = calculate_non_hp(base_stats.atk, ivs.atk, 0, lvl, nature_factors.atk);
                if actual_atk < min_stats.atk || actual_atk > max_stats.atk {
                    return false;
                }

                let actual_def = calculate_non_hp(base_stats.def, ivs.def, 0, lvl, nature_factors.def);
                if actual_def < min_stats.def || actual_def > max_stats.def {
                    return false;
                }

                let actual_spa = calculate_non_hp(base_stats.spa, ivs.spa, 0, lvl, nature_factors.spa);
                if actual_spa < min_stats.spa || actual_spa > max_stats.spa {
                    return false;
                }

                let actual_spd = calculate_non_hp(base_stats.spd, ivs.spd, 0, lvl, nature_factors.spd);
                if actual_spd < min_stats.spd || actual_spd > max_stats.spd {
                    return false;
                }

                let actual_spe = calculate_non_hp(base_stats.spe, ivs.spe, 0, lvl, nature_factors.spe);
