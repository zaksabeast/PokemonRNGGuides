// This file is a temporary near duplicate of chatterFilters.tsx
// while we test if users prefer relative pitches over absolute pitches
...
const markResultsWithChatter = ({
  ...
  const relativeChatters = getRelativeChatters(results);
  ...
  const markedResults = results.map((result, index): Result => {
    const adjustedIndex = index - 1; // -1 since relativeChatters is offset by 1 from results
    return {
      ...result,
      status:
  ...
  const endIndex = (hasMatch ? indicies[0].end : 0) + 1; // +1 to convert back from relativeChatters index to results index
  ...
  return (
    <Flex vertical gap={8}>
      ...
      <Flex gap={8}>
        <Button
          flex={1}
          trackerId="chatter_filter_lower"
        ...
        <Button
          flex={1}
          trackerId="chatter_filter_same"
          onClick={() => onClickPitch("S")}
        >
          Same
        </Button>
        <Button
          flex={1}
          trackerId="chatter_filter_higher"
...
export const RelativeChatterFilterBase = ({
  seed,
  targetAdvance,
  submitTrackerId,
